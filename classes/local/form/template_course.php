 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class template_course extends \core\form\persistent {
    ...
    protected function definition() {
        ...
        $mform->addElement('text', 'courseid', '', ['class' => 'hidden']);
        $mform->setType('courseid', PARAM_INT);

        $mform->addElement('selectyesno', 'display', get_string('display', 'quizaccess_sebprogram'), ['class' => 'hidden']);
        $mform->setType('display', PARAM_INT);

        $programid = $this->get_persistent()->get('id');

        $recordprograms = program::get_records_course(optional_param('course', -1, PARAM_INT), 'id', true);
        foreach ($recordprograms as $record) {
            ...
            if ( ($programid > 0 && $record->id != $programid) || ($programid == 0)  ) {
